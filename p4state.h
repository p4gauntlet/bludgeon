...
public:
    ...
    p4State() {
        if (P4Scope::sys_hdr->name.name != "Headers") {
            BUG("sys hdr name is not correct %1%!",
                P4Scope::sys_hdr->name.name);
        }
    ...
    IR::MethodCallStatement *gen_methodcall_stat(cstring field_name) {
        ...
        if (field_type->is<IR::Type_Stack>()) {
            ...
            if (tp_stack->elementType->is<IR::Type_Name>()) {
                ...
                auto real_tp = P4Scope::get_type_by_name(
                ...
                if (real_tp->is<IR::Type_HeaderUnion>()) {
                    ...
                    arg =
                        new IR::Argument(new IR::Member(arr_index,
                                                        IR::ID(sf->name.name)));
                } else {
            ...
            if (field_type->is<IR::Type_Name>()) {
                ...
                if (real_tp->is<IR::Type_HeaderUnion>()) {
                    ...
                    auto sf           = tp_hdr_union->fields.at(rand_ind);
                    arg =
                        new IR::Argument(new IR::Member(mem,
                                                        IR::ID(sf->name.name)));
                } else {
    ...
    inline IR::MethodCallStatement *gen_hdr_extract(IR::Member     *pkt_call,
                                                    IR::Expression *mem) {
        IR::Vector<IR::Argument> *args = new IR::Vector<IR::Argument>();
    ...
    inline void gen_hdr_union_extract(
            ...
            if (sf_type->is<IR::Type_Stack>()) {
                ...
                if (ele_tp->is<IR::Type_Header>()) {
                    ...
                    for (auto j = 0; j < size; j++) {
                        ...
                        auto arr_ind =
                            new IR::ArrayIndex(mem, new IR::Constant(j));
            ...
            case 3: {
                    ...
                    if (get_le_flag == true) {
                        for (int i = 0; i < rand() % 3 + 1; i++) {
                            ...
                            auto states_at_rnd = states.at(
                                rand() % states.size());
                            auto sw_case =
                                new IR::SelectCase(keyset,
