...
public:
    ...
    static IR::AssignmentStatement *gen_assignstat(bool if_in_parser = false) {
        ...
        if ((left != nullptr) && (right != nullptr)) {
            if (!if_in_parser ||
                (!left->is<IR::Mux>() && !right->is<IR::Mux>())) {
                ...
                if (l_size != r_size) {
                    ...
                    assignstat = new IR::AssignmentStatement(left, right_cast);
                } else {
    ...
    static IR::AssignmentStatement *gen_compound_ass() {
        ...
        if (expression::get_two_compound_operands(&expr1, &expr2, &tp1,
                                                  &tp2) == true) {
    ...
    static IR::MethodCallStatement *gen_act_methodcall_stat() {
        ...
        mcs =
            new IR::MethodCallStatement(new IR::MethodCallExpression(path_expr,
                                                                     args));
