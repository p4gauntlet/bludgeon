...
IR::Expression *expression2::gen_expr(const IR::Type *tp) {
    ...
    if (auto tb = tp->to<IR::Type_Bits>()) {
        ...
        /** stuff **/
        // not sure how to merge InfInt with booleans they are almost the same
...
IR::Expression *expression2::pick_lval(const IR::Type *tp) {
    IR::Expression *expr = nullptr;
...
IR::Expression *expression2::gen_input_arg(const IR::Parameter *param) {
    ...
    if (dir == IR::Direction::In) {
        ...
        return expression2::pick_lval(param->type);
    }
}
} // namespace CODEGEN
