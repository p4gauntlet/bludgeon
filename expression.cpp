...
const IR::Type *pick_field(std::map<cstring, const IR::Type *>& mp,
    ...
    if (tp->is<IR::Type_StructLike>()) {
        ...
        const IR::Type *ref_tp = P4Scope::get_type_by_name(
            t->path->name.name);
    ...
    for (auto i = call_bt.begin(); i < call_bt.end(); i++) {
        if (cnt == 0) {
            ...
            if (ret == -1) {
                ...
                expr =
                    new IR::ArrayIndex(expr,
                                       new IR::Constant(int_literal::gen(),
                                                        ret));
            }
        }
        cnt = cnt + 1;
...
void expression::initialize(const IR::Type                          *tp,
    ...
    if (tp->is<IR::Type_StructLike>()) {
        ...
        int size =
            (tp_bits->size <= SIZE_BIT_FOR_INITIALIZATION) ?
            tp_bits->size : SIZE_BIT_FOR_INITIALIZATION; // Tao: watch out for
                                                         // overflow
        auto val             = rand() % (2 << (size - 1));
        IR::Constant *r_expr = rand() % 2 == 0 ? new IR::Constant(
            tp_bits,
            val) : new IR::Constant(val);
        IR::AssignmentStatement *ass = new IR::AssignmentStatement(l_expr,
                                                                   r_expr);
...
std::vector<IR::AssignmentStatement *> expression::decl_initialize(
    std::map<cstring, const IR::Type *>& tp_map) {
...
IR::Expression *expression::get_operand(int            pa_or_var,
                                        const IR::Type **tp,
                                        cstring&       type,
                                        bool           if_compound,
                                        bool           if_param_in_inc) {
    ...
    if (pa_or_var == 0) {
        if (if_param_in_inc == true) {
            ...
            *tp = pick_lval(P4Scope::name_2_type_param,
                            call_backtrace,
                            type,
                            if_compound);
        }
    } else {
        *tp = pick_lval(P4Scope::name_2_type_vars,
                        call_backtrace,
                        type,
                        if_compound);
    }
...
IR::Expression *construct_slice(IR::Expression *expr1) {
    ...
    if (expression::forbidden_exprs.find(expr1) !=
        expression::forbidden_exprs.end()) {
        return nullptr;
    }
    ...
    expression::mp_expr_2_type.emplace(expr,
                                       new IR::Type_Bits(l_range - r_range + 1,
                                                         false));
...
IR::Expression *construct_mux(IR::Expression *expr1, IR::Expression *expr2) {
    ...
    if ((expression::forbidden_exprs.find(expr1) !=
         expression::forbidden_exprs.end()) ||
        (expression::forbidden_exprs.find(expr2) !=
         expression::forbidden_exprs.end())) {
        return nullptr;
    }
...
template<typename T>
IR::Expression *construct_2_ops(IR::Expression *expr1,
                                IR::Expression *expr2,
                                bool           if_cast       = true,
...
IR::Expression *construct_div(IR::Expression *expr1) {
    ...
    int val = 1 <<
              (rand() %
               (size <=
                SIZE_BIT_FOR_INITIALIZATION ? size :
                SIZE_BIT_FOR_INITIALIZATION));

    expr =
        new IR::Div(expr1,
                    new IR::Constant(new IR::Type_Bits(size, false), val));
    expression::mp_expr_2_type[expr] = expression::mp_expr_2_type[expr1];
    expression::bit_exprs.push_back(expr);
    return expr;
}
    ...
    while (num_trials--) {
        switch (rand() % 16) {
            ...
            case 2:
                ...
                expr = nullptr; // construct_slice(expr1);
                break;
            case 3:  expr = construct_2_ops<IR::Mul>(expr1,
                                                     expr2,
                                                     true,
                                                     is_const_expr);
                break;
            case 4:  expr = construct_div(expr1);
                break;
            case 5:  expr = construct_2_ops<IR::Add>(expr1,
                                                     expr2,
                                                     true,
                                                     is_const_expr);
                break;
            case 6:  expr = construct_2_ops<IR::Sub>(expr1,
                                                     expr2,
                                                     true,
                                                     is_const_expr);
                break;
            case 7:  expr = construct_2_ops<IR::AddSat>(expr1,
                                                        expr2,
                                                        true,
                                                        is_const_expr);
                break;
            case 8:  expr = construct_2_ops<IR::SubSat>(expr1,
                                                        expr2,
                                                        true,
                                                        is_const_expr);
                break;
            case 9:  expr = construct_2_ops<IR::BAnd>(expr1,
                                                      expr2,
                                                      true,
                                                      is_const_expr);
                break;
            case 10: expr = construct_2_ops<IR::BOr>(expr1,
                                                     expr2,
                                                     true,
                                                     is_const_expr);
                break;
            case 11: expr = construct_2_ops<IR::BXor>(expr1,
                                                      expr2,
                                                      true,
                                                      is_const_expr);
                break;
            ...
            case 13: expr = construct_2_ops<IR::Shl>(expr1,
                                                     ...
                                                     , false, is_const_expr);
                break;
            case 14: expr = construct_mux(expr1, expr2);
                break;
            case 15: expr = construct_mod(expr1);
                break;
        }
...
IR::Expression *expression::get_cond_expr() {
    ...
    if (is_from_vector == false) {
        if (type == EXPR_TYPE_BOOL) {
            switch (rand() % 2) {
                case 0: expr = new IR::Equ(expr, bool_literal::gen_literal());
                    break;

                // case 1: expr = new IR::Geq(expr,
                // bool_literal::gen_literal()); break;
                // case 2: expr = new IR::Grt(expr,
                // bool_literal::gen_literal()); break;
                // case 3: expr = new IR::Leq(expr,
                // bool_literal::gen_literal()); break;
                // case 4: expr = new IR::Lss(expr,
                // bool_literal::gen_literal()); break;
                case 1: expr = new IR::Neq(expr, bool_literal::gen_literal());
                    break;
            }
...
IR::Expression *expression::construct_cond_expr() {
    ...
    switch (rand() % 3) {
        case 0: expr = new IR::LNot(expr1);
            break;
        case 1: expr = new IR::LAnd(expr1, expr2);
            break;
        case 2: expr = new IR::LOr(expr1, expr2);
            break;
    }
...
IR::Expression *get_compound_operand(const IR::Type **tp,
                                     cstring        compound_type,
                                     bool           if_param_in_inc) {
    IR::Expression *expr = nullptr;
    cstring type;
    int num_trials = 100;
    ...
    while (num_trials--) {
        expr = expression::get_operand(rand() % 2,
                                       tp,
                                       type,
                                       true,
                                       if_param_in_inc);
...
bool expression::get_two_compound_operands(IR::Expression **expr1,
                                           IR::Expression **expr2,
                                           const IR::Type **tp1,
                                           const IR::Type **tp2) {
    bool ret       = false;
...
IR::Expression *expression::construct_compound_cond_expr() {
    ...
    if (get_two_compound_operands(&expr1, &expr2, &tp1, &tp2) == true) {
        switch (rand() % 2) {
            case 0: expr = new IR::Neq(expr1, expr2);
                break;
            case 1: expr = new IR::Equ(expr1, expr2);
                break;
        }
...
IR::Vector<IR::Argument> *expression::construct_params(
    ...
    if (tp->is<IR::Type_StructLike>()) {
        ...
        *if_contains_stack = true;
        ...
        auto size    = (tp_bits->size <= SIZE_BIT_FOR_INITIALIZATION) ?
                       tp_bits->size : SIZE_BIT_FOR_INITIALIZATION; // Tao:
                                                                    // watch out
                                                                    // for
                                                                    // overflow
        auto val  = rand() % (2 << (size - 1));
        auto expr = rand() % 2 ==
                    0 ? new IR::Constant(tp_bits, val) : new IR::Constant(val);
        exprs.push_back(expr);
    } else if (tp->is<IR::Type_Boolean>()) {
        auto expr = rand() % 2 ==
                    0 ? new IR::BoolLiteral(false) : new IR::BoolLiteral(true);
        exprs.push_back(expr);
    }
}
...
IR::Expression *expression::construct_func_call_expr() {
    ...
    if (funcs.size() != 0) {
        ...
        ret =
            new IR::MethodCallExpression(new IR::PathExpression(new IR::Path(
                                                                    func
                                                                        ->
                                                                        name)),
                                         args);
