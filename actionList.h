...
public:
	...
	IR::ActionList* gen(size_t len) {
		...
		std::map<cstring, std::vector<const IR::Type*>> p4act_def = P4Scope::get_action_def();
        ...
        for (size_t i=0; i<len; i++) {
			...
			IR::MethodCallExpression* mc_expr = new IR::MethodCallExpression(path_expr, args);
            IR::ActionListElement* actlist_ele = new IR::ActionListElement(mc_expr);
